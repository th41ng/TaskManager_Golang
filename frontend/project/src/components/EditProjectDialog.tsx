  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/projects/${project.id}`, {
        ...
        body: JSON.stringify({ id: project.id, name, owner_id: project.owner_id }),
      })
    ...
    >
      ...
      >
        <div className="flex items-center justify-between p-6 border-b border-gray-200 dark:border-gray-700">
          <div>
            <h2 className="text-2xl font-bold text-gray-900 dark:text-white">✏️ Edit Project</h2>
            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Update project details</p>
          </div>
          <button
            type="button"
            onClick={() => onOpenChange(false)}
